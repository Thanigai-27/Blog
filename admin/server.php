<?php 
	...
	if (isset($_POST['sign'])) {
		...
		if (count($errors) == 0) {
			...
			$sql =mysqli_query($con,"INSERT INTO admin (username,email,password,name,contact_no) VALUES ('$username', '$email','$password','$name','$contact')");

			$_SESSION['login'] = $username;
			$user_id = mysqli_insert_id($con); 

			// put logged in user into session array
			$_SESSION['admin'] = getUserById($user_id);
